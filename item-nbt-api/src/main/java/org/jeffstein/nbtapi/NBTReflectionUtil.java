...
import org.jeffstein.nbtapi.utils.GsonWrapper;
...
import org.jeffstein.nbtapi.utils.nmsmappings.CodecHelper;
import org.jeffstein.nbtapi.utils.nmsmappings.ObjectCreator;
...
public class NBTReflectionUtil {
    ...
    static {
        ...
        if (MinecraftVersion.isAtLeastVersion(MinecraftVersion.MC1_20_R4)) {
            try {
                Field typeField = ReflectionUtil.getMappedField(ClassWrapper.NMS_DATACOMPONENTS.getClazz(),
            ...
            try {
                ...
                itemstack_codec = (Codec<Object>) ReflectionUtil.getMappedField(ClassWrapper.NMS_ITEMSTACK.getClazz(),
                        "net.minecraft.world.item.ItemStack#CODEC").get(null);
                nbtOps = (DynamicOps<Object>) ReflectionUtil
                        .getMappedField(ClassWrapper.NMS_NBTOPS.getClazz(), "net.minecraft.nbt.NbtOps#INSTANCE")
                        .get(null);
                if (MinecraftVersion.isAtLeastVersion(MinecraftVersion.MC1_21_R5)) {
                    nbtRegistryOps = (DynamicOps<Object>) ReflectionMethod.GET_SERIALIZATION_CONTEXT
                            .run(registry_access, nbtOps);
                    problemReporter = ReflectionUtil.getMappedField(ClassWrapper.NMS_PROBLEM_REPORTER.getClazz(),
                            "net.minecraft.util.ProblemReporter#DISCARDING").get(null);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public static Object setEntityNBTTag(Object nbtTag, Object nmsEntity) {
        try {
            if (MinecraftVersion.isAtLeastVersion(MinecraftVersion.MC1_21_R5)) {
                Object valueInputTag = ReflectionMethod.NMS_GET_TAG_VALUE_INPUT.run(null, problemReporter,
                        registry_access, nbtTag);
    ...
    public static void setTileEntityNBTTagCompound(BlockState tile, Object comp) {
        try {
            ...
            if (MinecraftVersion.isAtLeastVersion(MinecraftVersion.MC1_21_R5)) {
                Object valueInput = ReflectionMethod.NMS_GET_TAG_VALUE_INPUT.run(null, problemReporter, registry_access,
                        comp);
